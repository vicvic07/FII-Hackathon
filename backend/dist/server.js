...
function publicStreakShare(token) {
...
app.get('/public/streak-shares/:token', (req, res) => {
    ...
    if (!shared)
        return res.status(404).json({ error: 'STREAK_SHARE_NOT_FOUND' });
    res.json(shared);
});
app.get('/share/:token', (req, res) => {
    const shared = publicStreakShare(req.params.token);
    if (!shared)
        return res.status(404).type('html').send('<h1>This streak link is no longer available.</h1>');
...
app.post('/v1/conversations', (req, res) => {
    ...
    if (kind === 'PEER' && (req.actor.role !== 'USER' || participant.role !== 'USER'))
        return res.status(400).json({ error: 'PEER_REQUIRED', message: 'Peer chat is available only between user accounts.' });
...
app.post('/v1/peer-connections', (req, res) => {
    ...
    if (!peer || peer.id === req.actor.id || req.actor.role !== 'USER' || peer.role !== 'USER')
        return res.status(400).json({ error: 'PEER_REQUIRED', message: 'Choose another user account for peer support.' });
    ...
    if (existing)
        return res.json(existing);
    const conversation = { id: makeId('conv'), kind: 'PEER', memberIds: [req.actor.id, peer.id], startedAt: new Date(), billedSeconds: 0 };
    store.conversations.push(conversation);
    res.status(201).json(conversation);
});
