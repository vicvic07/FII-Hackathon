...
const oauthTransactions = new Map<string, ReturnType<typeof newOAuthTransaction>>()
...
app.get('/auth/google', (_, res) => {
  try { const tx = newOAuthTransaction(); oauthTransactions.set(tx.state, tx); res.redirect(googleAuthorizationUrl(tx)) }
  catch { res.status(503).json({ error: 'GOOGLE_OAUTH_NOT_CONFIGURED', message: 'Set Google OAuth configuration in backend/.env.' }) }
})
app.get('/auth/google/callback', async (req, res) => {
  const code = typeof req.query.code === 'string' ? req.query.code : undefined
  const state = typeof req.query.state === 'string' ? req.query.state : undefined
  const tx = state && oauthTransactions.get(state)
  if (!code || !tx || tx.expiresAt < Date.now()) return res.status(400).send('Invalid or expired sign-in request.')
  ...
  } catch { res.status(401).send('Google sign-in could not be verified. Please try again.') }
})
...
app.post('/auth/onboarding', (req, res) => {
  ...
  const actor = req.actor!; actor.role = parsed.data.role; actor.onboardingComplete = true
  if (actor.role === 'PROFESSIONAL' && !store.therapist(actor.id)) store.therapists.push({ id: actor.id, name: actor.name, specialties: [], hourlyRateCents: 0, acceptingClients: false, verified: false })
  res.json(actor)
})
